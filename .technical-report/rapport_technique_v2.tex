...
Le projet s'inscrit dans le cadre du module \emph{DevOps \& MLOps} et correspond au Projet~9 du guide \og Classification de Texte avec Pipeline CI/CD Complet \fg{}. Un journal en ligne souhaite automatiser la catégorisation de ses articles en fonction de leur contenu, afin de faciliter la navigation, la recommandation et l'analyse éditoriale.
...
Selon l'énoncé du Projet~9, le contexte est celui d'un journal en ligne qui veut automatiser la catégorisation de ses articles. Chaque texte doit être attribué à une catégorie thématique (informatique, sport, politique, religion, etc.), ce qui permet :
...
\begin{itemize}
  ...
  \item de préparer des cas d'usage en aval (recherche, personnalisation).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Chargement}~: via \texttt{sklearn.datasets.fetch\_20newsgroups}, en supprimant en-têtes, pieds de page et citations afin de limiter les biais.
\end{itemize}
...
\begin{itemize}
  \item une performance de classification correcte (par exemple accuracy et F1 pondérée significatives) sur le jeu de test ;
