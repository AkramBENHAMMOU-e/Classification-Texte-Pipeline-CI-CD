...
% -----------------------------------------------------------------------------
\begin{frame}{Workflow CML - Continuous Machine Learning}
    \centering
    \begin{tikzpicture}[
        node distance=0.8cm,
        stage/.style={draw, rounded corners, fill=#1, minimum width=3.2cm, minimum height=1.2cm, align=center, font=\small},
        arrow/.style={->, thick, primaryBlue}
    ]
        % Trigger
        \node[stage=githubPurple!30] (trigger) at (0,0) {\faCodeBranch\\ git push master};
        
        % Setup
        \node[stage=cyan!30] (setup) at (4,0) {\faCog\\ Setup CML\\[-0.1cm] \footnotesize iterative/setup-cml};
        
        % Preprocess
        \node[stage=yellow!30] (preprocess) at (8,0) {\faDatabase\\ Prétraitement\\[-0.1cm] \footnotesize preprocess.py};
        
        % Train
        \node[stage=orange!30] (train) at (0,-2.5) {\faBrain\\ Entraînement\\[-0.1cm] \footnotesize train.py + MLflow};
        
        % Metrics
        \node[stage=mlopsGreen!30] (metrics) at (4,-2.5) {\faChartBar\\ Métriques\\[-0.1cm] \footnotesize JSON + Images};
        
        % Report
        \node[stage=accentCyan!30] (report) at (8,-2.5) {\faComment\\ Rapport PR\\[-0.1cm] \footnotesize cml comment};
        
        % Arrows
        \draw[arrow] (trigger) -- (setup);
        \draw[arrow] (setup) -- (preprocess);
        \draw[arrow] (preprocess) -- (train);
    ...
    \begin{columns}
        \column{0.5\textwidth}
        \begin{block}{\footnotesize Éléments du Rapport}
            \footnotesize
            \begin{itemize}
                \item[\faFileAlt] Métriques JSON (accuracy, f1)
                \item[\faImage] Matrice de confusion PNG
                \item[\faFileCode] Rapport classification texte
            \end{itemize}
        \end{block}
        
        \column{0.5\textwidth}
        \begin{alertblock}{\footnotesize Avantage CML}
            \footnotesize Visibilité des performances ML directement dans les Pull Requests GitHub
        \end{alertblock}
    \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
\begin{frame}{Workflow Docker - Build \& Push}
    \centering
    \begin{tikzpicture}[
        node distance=0.8cm,
        stage/.style={draw, rounded corners, fill=#1, minimum width=3cm, minimum height=1.2cm, align=center, font=\small},
        arrow/.style={->, thick, primaryBlue}
    ]
        % Trigger
        \node[stage=githubPurple!30] (trigger) at (0,0) {\faCodeBranch\\ git push master};
        ...
        % Tests
        \node[stage=cyan!30] (tests) at (4,0) {\faVial\\ Tests pytest\\[-0.1cm] \footnotesize pytest tests/ -v};
        
        % Build
        \node[stage=dockerBlue!30] (build) at (8,0) {\faDocker\\ Build Image\\[-0.1cm] \footnotesize Dockerfile};
        
        % Login
        \node[stage=yellow!30] (login) at (0,-2.5) {\faKey\\ Login GHCR\\[-0.1cm] \footnotesize docker/login-action};
        
        % Push
        \node[stage=orange!30] (push) at (4,-2.5) {\faCloudUploadAlt\\ Push Image\\[-0.1cm] \footnotesize ghcr.io/...};
        
        % Tags
        \node[stage=mlopsGreen!30] (tags) at (8,-2.5) {\faTags\\ Tags\\[-0.1cm] \footnotesize sha + latest};
        
        % Arrows
        \draw[arrow] (trigger) -- (tests);
        \draw[arrow] (tests) -- (build);
        \draw[arrow] (build) -- (login);
        \draw[arrow] (login) -- (push);
        \draw[arrow] (push) -- (tags);
    \end{tikzpicture}
    ...
    \begin{columns}
        \column{0.5\textwidth}
        \begin{block}{\footnotesize Dockerfile Optimisé}
            \footnotesize
            \begin{itemize}
                \item[\faBox] Image \texttt{python:3.9-slim}
                \item[\faCogs] Cache des dépendances
                \item[\faBook] Ressources NLTK
                \item[\faRocket] Uvicorn port 8000
            \end{itemize}
        \end{block}
        
        \column{0.5\textwidth}
        \begin{alertblock}{\footnotesize Registry GHCR}
            \footnotesize
            \begin{itemize}
                \item GitHub Container Registry
                \item Tags: SHA commit + latest
                \item Multi-arch: amd64 + arm64
            \end{itemize}
        \end{alertblock}
    \end{columns}
\end{frame}
...
\begin{frame}{CML - Continuous Machine Learning}
    \begin{columns}
        ...
        \includegraphics[width=5cm]{rapport-cml-genere.png}
    \end{columns}
\end{frame}
...
\begin{frame}{Angular 21 - Interface Utilisateur}
    ...
    \includegraphics[width=6cm,height=3.5cm,keepaspectratio]{ui-demo.png}
...
\begin{frame}{Performances du Modèle}
    \begin{columns}
        ...
        \includegraphics[width=5.5cm]{confusion_matrix.png}
    \end{columns}
\end{frame}
